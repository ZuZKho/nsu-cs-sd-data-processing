...
(defn getNewState [state key amount]
  (let
    [nw-state (update state :buffer (fn [old new] assoc old new) (update (state :buffer) key + amount))]
      nw-state))

(defn getDiff [state] (let [diff (assoc state :buffer (merge-with - (state :buffer) (state :bill)))]
                        diff))

(defn start-factory-buffer [state key amount storage-atom]
  (let [old-state state,
        nw-state (getNewState state key amount),
        diff (getDiff nw-state),
        have-to-take (max 0 (min @storage-atom (- 0 ((diff :buffer) key))))] ; Take as much as we can
    (try
      (swap! storage-atom #(- % have-to-take))
      (if (every? #(>= % 0) (vals (diff :buffer)))
        (do
          (Thread/sleep (state :duration))
          (send ((state :target-storage) :worker) supply-msg (state :amount))
          diff)
        nw-state)
       (catch IllegalStateException e old-state))
    ))
...
(defn notify-msg
  ...
  (let [diff (merge-with - (state :buffer) (state :bill))]
    (if (every? #(>= % 0) (vals diff))
      state
      (start-factory-buffer state ware amount storage-atom))))
